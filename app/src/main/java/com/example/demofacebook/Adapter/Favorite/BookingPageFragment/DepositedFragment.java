        ...
        ApiService.apiService.geOrderIdByUser().enqueue(new Callback<List<OrderInformation>>() {
            ...
            public void onResponse(Call<List<OrderInformation>> call, Response<List<OrderInformation>> response) {
                if (response.isSuccessful()) {
                    ...
                    orderList = value.stream().filter(p->p.getStatus().equals("deposited")).collect(Collectors.toList());
...
//        myList.add(new Order(1, "Studio Name", dateChange, "DEPOSITED", 100000, 4, "", "Ten service dau tien", detailList));
//        myList.add(new Order(2, "Studio Name", dateChange, "DEPOSITED", 100000, 4, "", "Ten service dau tien", detailList));
//        myList.add(new Order(3, "Studio Name", dateChange, "DEPOSITED", 100000, 4, "", "Ten service dau tien", detailList));
//        myList.add(new Order(4, "Studio Name", dateChange, "DEPOSITED", 100000, 4, "", "Ten service dau tien", detailList));
//        myList.add(new Order(5, "Studio Name", dateChange, "DEPOSITED", 100000, 4, "", "Ten service dau tien", detailList));
//        return myList;
//    }
